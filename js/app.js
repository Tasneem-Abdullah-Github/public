...
const showProducts = (products) => {
  ...
  for (const product of allProducts) {
    ...
    <img class="product-image" src=${image}></img>
      ...
      <h4>${showRatings(product.rating.rate)} Ratings: ${product.rating.rate}</h4>
      <h4>Rated: (${product.rating.count})</h4>
      <button onclick="addToCart(${product.id},${product.price})" id="addToCart-btn" class="buy-now btn btn-success">add to cart</button>
      <a href = "#search-area" id="details-btn" class="btn btn-danger" onclick = "loadSingleData(${product.id})">Details</a></div>
      `;
...
const showSingleData = product =>{
  const details = document.getElementById("details-display");
  details.classList.add("details")
  // Set innerHTML of Each Single Product
  details.innerHTML = `
  <img src = ${product.image} width = "200px" class = "d-block mx-auto"></img>
  <h3>${product.title}</h3>
  <p>${product.category}</p>
  <p>${product.description}</p>
  <h3>Price: ${product.price}</h3>
  <p>${showRatings(product.rating.rate)} Ratings: ${product.rating.rate}</p>
